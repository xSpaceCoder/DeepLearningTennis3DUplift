...
class UpliftingModel:
    def __init__(self):
        ...
        model_path = os.path.join(paths.weights_path, "rallies_dynamicAnkle", "model.pt")
        self.model, self.transform, self.transform_mode = load_uplifting_model(model_path=model_path)
    ...
    def predict(self, ball_coords, court_courts, times):
        ...
        return self.predict_without_normalization(ball_coords, court_courts, torch.tensor(mask).to(self.device), times)
    ...
    def predict_without_normalization(self, ball_coords, court_courts, mask, times):
        ...
        '''
        ball_coords, court_courts, mask, times = ball_coords.to(self.device), court_courts.to(self.device), mask.to(self.device), times.to(self.device)

        with torch.no_grad():
            pred_rotation, pred_position = self.model(ball_coords, court_courts, mask, times)

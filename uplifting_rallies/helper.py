...
""" 
def get_data_path():
    tmp_exists = os.path.exists(DATA_PATH_SYNTH_RALLIES)
    if tmp_exists:
        return os.path.join(DATA_PATH_SYNTH_RALLIES)
    else:
        return os.path.join(DATA_PATH, "stitched_rallies_02-24") """
...
def binary_metrics(rot_gt, rot_pred):
    ...
    Args:
        rot_gt (torch.tensor): ground truth rotation, shape (B, 3) or (B, T, 3)
        rot_pred (torch.tensor): predicted rotation, shape (B, 3) or (B, T, 3)
    Returns:
    ...
    """
    assert len(rot_gt.shape) == len(
        rot_pred.shape
    ), "rot_gt and rot_pred must have the same shape"
    assert len(rot_gt.shape) in [
        2,
        3,
    ], "rot_gt and rot_pred must have shape (B, 3) or (B, T, 3)"

    rot_gt = torch.sign(rot_gt)
    rot_pred = torch.sign(rot_pred)
    TP = torch.logical_and(rot_gt[:, :] == 1, rot_pred[:, :] == 1)
    TN = torch.logical_and(rot_gt[:, :] == -1, rot_pred[:, :] == -1)
    FP = torch.logical_and(rot_gt[:, :] == -1, rot_pred[:, :] == 1)
    FN = torch.logical_and(rot_gt[:, :] == 1, rot_pred[:, :] == -1)

    if len(rot_gt.shape) == 3:  # Shape (B, T, 3)
        TP = torch.mean(TP.float(), dim=1)
        TN = torch.mean(TN.float(), dim=1)
        FP = torch.mean(FP.float(), dim=1)
        FN = torch.mean(FN.float(), dim=1)
...
def transform_rotationaxes(rotation, r_gt, mask=None):
    """Rotates the axes such that the x axis is in flight direction, y axis in the worlds x-y plane.

    Computes per-frame flight directions using forward differences to correctly handle
    rallies with multiple shots going in different directions.

    Args:
        rotation (torch.tensor): rotation of the ball, shape (B, T, 3)
        r_gt (torch.tensor): ground truth or predicted trajectory, shape (B, T, 3)
        mask (torch.tensor, optional): validity mask, shape (B, T). If provided, frames with
            mask=0 will have their direction propagated from the nearest valid preceding frame.
    Returns:
        torch.tensor: rotation in local coordinate system, shape (B, T, 3)
    """
    assert len(r_gt.shape) == 3, "r_gt must have shape (B, T, 3)"
    B, T, _ = r_gt.shape
    device = r_gt.device
    dtype = r_gt.dtype

    # e_z is constant (vertical world axis) for all frames
    e_z = torch.zeros((B, T, 3), device=device, dtype=dtype)
    e_z[..., 2] = -1.0

    # Compute per-frame velocity using forward difference
    # velocity[t] = r_gt[t+1] - r_gt[t] for t < T-1
    velocity = torch.zeros((B, T, 3), device=device, dtype=dtype)
    if T > 1:
        velocity[:, :-1, :] = r_gt[:, 1:, :] - r_gt[:, :-1, :]
        velocity[:, -1, :] = velocity[:, -2, :]  # Last frame copies previous velocity
    else:
        velocity[:, 0, 0] = 1.0  # Default direction for single-frame case

    # Project to x-y plane (horizontal flight direction)
    v_xy = velocity.clone()
    v_xy[..., 2] = 0.0

    # Compute norms for normalization
    v_norm = torch.linalg.norm(v_xy, dim=-1, keepdim=True)  # (B, T, 1)

    # Normalize with clamping for numerical stability
    e_x = v_xy / torch.clamp(v_norm, min=1e-8)

    # Handle masked/invalid frames by forward-filling valid directions
    # This ensures frames with zero positions (padding) get a reasonable direction
    if mask is not None:
        # Combine velocity validity with external mask
        invalid = (v_norm.squeeze(-1) < 1e-8) | (mask == 0)

        # Forward-fill: propagate last valid direction to invalid frames
        # Vectorized over batch, loop over time (O(T) operations)
        last_valid = torch.zeros((B, 3), device=device, dtype=dtype)
        last_valid[:, 0] = 1.0  # Default: positive x direction

        for t in range(T):
            valid_t = ~invalid[:, t]  # (B,) - which batches have valid frame at t
            # Update last_valid where current frame is valid
            last_valid = torch.where(valid_t.unsqueeze(-1), e_x[:, t, :], last_valid)
            # For invalid frames, use the propagated direction
            e_x[:, t, :] = torch.where(
                invalid[:, t].unsqueeze(-1), last_valid, e_x[:, t, :]
            )

    # e_y = e_z cross e_x (perpendicular in x-y plane)
    e_y = torch.cross(e_z, e_x, dim=-1)

    # Transform rotation to local coordinate system using per-frame basis vectors
    w_0 = torch.einsum("bti,bti->bt", rotation, e_x)
    w_1 = torch.einsum("bti,bti->bt", rotation, e_y)
    w_2 = torch.einsum("bti,bti->bt", rotation, e_z)
...
def inversetransform_rotationaxes(local_rotation, r_gt):
    ...
    e_x = v0 / torch.clamp(torch.linalg.norm(v0, dim=-1, keepdim=True), min=1e-8)
    e_y = torch.cross(e_z, e_x, dim=-1)
